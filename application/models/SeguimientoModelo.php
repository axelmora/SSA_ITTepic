...
class SeguimientoModelo extends CI_Model {
  ...
  public function cargarEncuestasSeguimiento($idSeguimiento)
  {
    ...
    public function cargarGrupoId($idrupo)
    {
      ...
      $query=$DBcon->query("SELECT * FROM  resultados_seguimiento as rs, alumnos as al, carreras as ca where encuestas_seguimiento_idencuesta_seguimiento=$idrupo and rs.no_de_control=al.numero_control and al.carreras_id_carrera=ca.id_carrera;
        ");
        if ($query->num_rows() > 0) {
          return $query->result();
        } else {
      ...
      public function cargarDoceneteGrupo($idrupo)
      {
        ...
        {
          $DBcon = $this->load->database('default', TRUE);
          $query=$DBcon->query("SELECT * FROM  aplicaciones  where periodos_escolares_idperiodos='$Periodo'  and departamento_academico_iddepartamento_academico=$Departamento order by fecha_creacion asc; ");
          if ($query->num_rows() > 0)
          {
            return $query->result();
          } else {
        ...
        public function verificarAplicacionExterna($Periodo,$Departamento)
        {
          $DBcon = $this->load->database('default', TRUE);
...
{
  $DBcon = $this->load->database('default', TRUE);
  $DBcon->set('respuestas','NULL', false);
